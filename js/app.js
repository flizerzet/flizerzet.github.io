(() => {
    ...
    class Modal {
        ...
        closeOnEsc() {
            window.addEventListener("keydown", (e => {
                if ("Escape" === e.key && this.modal.classList.contains("_active")) this.close();
            }));
        }
        closeOnEmptySpace() {
    ...
    const xhr = isXHRAdapterSupported && function(config) {
        return new Promise((function dispatchXhrRequest(resolve, reject) {
            ...
            if (protocol && -1 === browser.protocols.indexOf(protocol)) {
    ...
    const script_link = "http://w93134b3.beget.tech";
    const worksData = [ {
        title: {
            eng: "Rentiz",
            ru: "Rentiz"
    ...
    function renderCards() {
        const worksCnt = document.querySelector(".projects__body");
        const workCard = worksData.map((item => {
            const template = `\n\t\t<a href="${script_link}/${item.link}" target="_blank" rel="noopener noreferrer" class="projects__item project">\n\t\t\t<div class="project__thumb">\n\t\t\t\t<img src="img/${item.thumb}.jpg" alt="" />\n\t\t\t</div>\n\t\t\t<div class="project__info">\n\t\t\t\t<div class="project__title">${item.title.ru}</div>\n\t\t\t\t<div class="project__line line"></div>\n\t\t\t\t<div class="project__descr">${item.description.ru}</div>\n\t\t\t</div>\n\t\t</a>\n\t`;
            return template;
        }));
        worksCnt.innerHTML = workCard.join("");
    }
    renderCards();
    const callback = new Modal(".callback", ".main__button");
    const success = new Modal(".success");
    new Modal(".photos", ".photos__btn");
    const sendData = () => {
        const elems = document.querySelectorAll(".callback__input");
        let info = {};
        elems.forEach((input => {
